...
# 最终精确提取umac_disc数据的脚本
# 基于找到的umac_disc开始标记和计算出的结束位置

INPUT_FILE="/Users/eggfly/github/pico-mac-all/pico-mac-nano/firmware/firmware_3_2.bin"
OUTPUT_FILE="extracted_correct_disc_image.bin"
...
# 基于分析得到的位置
DISC_START_OFFSET=430576  # 0x691f0 - 找到的umac_disc开始位置
...
# 验证结束位置的数据
echo ""
echo "验证umac_disc结束位置的数据..."
DISC_END_DATA=$(hexdump -C "$INPUT_FILE" | grep -A 2 -B 2 "$(printf '%08x' $DISC_END_OFFSET)" | head -5)
echo "umac_disc结束位置数据:"
echo "$DISC_END_DATA"
